...
ECHO %LENSES%
cl /LD /I %LENTIL_BUILD_HOME%\arnold\Arnold-5.2.0.0-windows\include /I %LENTIL_BUILD_HOME%\lentil\Eigen\Eigen /I /I %LENTIL_BUILD_HOME%\lentil\polynomial-optics\src /EHsc /O2 %LENSES% %LENTIL_BUILD_HOME%\lentil\pota\src\pota.cpp /link /LIBPATH:%LENTIL_BUILD_HOME%\arnold\Arnold-5.2.2.0-windows\lib ai.lib /OUT:%LENTIL_BUILD_HOME%\builds\%USER_BUILD_DIR%\lentil.dll
